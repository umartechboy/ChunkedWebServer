...
void WebServer::_prepareHeader(String& response, int code, const char* content_type, size_t contentLength) {
  ...
  /*  if (_contentLength == CONTENT_LENGTH_NOT_SET) {
		...
		if(_contentLength == CONTENT_LENGTH_UNKNOWN/* && _currentVersion*/){ //HTTP/1.1 or above client
      //let's do chunked
      _chunked = true;
...
void WebServer::sendFileAsChunk(const char* fileName)
{
	...
	while (tRead > 0)
	{
		//Serial.printf("Chunk: tread = %d, ", tRead);
		sendChunk(buffer, tRead);
		//Serial.printf("Sent\n");
		tRead = f.read(buffer, 5000);
	}
	f.close();
}
//OK		   
void WebServer::sendChunkP(const char* PGMString)
{
	uint8_t buffer[1000];
	size_t crsr = 0;
	long left = strlen_P(PGMString);;
	while (left > 0)
	{
		int thisPacket = min(left, (long)1000);
		for (int i = 0; i < thisPacket; i++)
		{
			buffer[i] = pgm_read_byte(PGMString + crsr);
			crsr++;
		}
		left -= thisPacket;
		sendChunk(buffer, thisPacket);
	}
}
void WebServer::beginChunkedResponse()
{
	beginChunkedResponse(String(F("text/html")));
}
//OK
void WebServer::beginChunkedResponse(String content_type)
{
...
void WebServer::sendChunk(String str)
{
	sendChunk((uint8_t*)str.c_str(), str.length());
}
void WebServer::sendChunk(uint8_t* data, int length)
{
	client().print(String(length, HEX));
	client().print(F("\r\n"));
	//client().setTimeout(length / 10000 + 2);
	//Serial.printf("timeout: %d \n", length / 10000 + 2);
	client().write(data, length);
	client().print(F("\r\n"));
}
void WebServer::endChunkedResponse()
{
	client().print(F("0\r\n\r\n"));
}
